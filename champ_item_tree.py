...
def item_rank():
    df = pd.DataFrame(columns=['champion_id', 'win_or_lose', 'lane', 'gold_earn', 'cs', 'kill', 'assist', 'lose_cnt'])
    ...
    for row in rows:#모든 row 보기
        ...
        if search.find_by_attr(start, row[3], maxlevel=2) :#중복되는 챔피언 번호가 있다면
            ...
            if search.find_by_attr(yeschamp, row[2], maxlevel=2):#중복되는 챔피언번호에 중복되는 라인이 있다면
                ...
                yeslane.cnt += 1
                if search.find_by_attr(yeslane, row[1], maxlevel=2):#중복되는 챔피언번호에 중복되는 라인에 중복되는 승패가 있다면
    ...
    for pre, fill, node in RenderTree(start):
        ...
        if itemtree.count('/') > 6:
            ...
            #print(listlist)
            sql = 'INSERT INTO item_rank(champion_id, lane, item_1, item_2, item_3, play_champ_num) values(%s, %s, %s, %s, %s, %s)'

    df = df.groupby(['champion_id', 'lane', 'item_1', 'item_2', 'item_3']).agg({'win_cnt': 'first',
                                                                                'lose_cnt': 'last',
                                                                                'play_champ_num':'first'}).reset_index()
